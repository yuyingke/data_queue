...
{
	return DATA_FIFO_NUM - 1 - data_fifo_get_write_num(fifo);
}
...
uint16_t data_fifo_write_data(DATA_FIFO_T *fifo, uint8_t *data, uint16_t data_len)
{
	...
	if(fifo_empty_num > 0)
	{
		...
		fifo->data[fifo->index_w].len = len;
...
{
	uint16_t len = 0;
	uint16_t fifo_write_num = data_fifo_get_write_num(fifo);

	if(fifo_write_num > 0)
	{
		// 依赖 data_fifo_write_data 函数检查
		if(DATA_BUF_SIZE < data_len)
		{
			data_len = DATA_BUF_SIZE;
			len = data_len;
